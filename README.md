...
| Open Platform Communications </br>(OPC)    | OPC can be devided into server and client. Server will be the one to communicate </br>with FCS, while client will be the one to ask server to communicate and then receive data in FCS. Client can </br>then process these data. There are multiple vendor (such as Kepware, OPC Expert, Advosol) who can provide OPC </br>server software and corresponding library for development.    
